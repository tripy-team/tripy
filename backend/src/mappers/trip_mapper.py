...
def trip_storage_to_response(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "flexibility_days": item.get("flexibilityDays"),
        "leg_dates": item.get("legDates"),  # Multi-city leg dates
        "flexibility_summary": item.get("flexibilitySummary"),
        ...
        "include_budget_airlines": item.get("includeBudgetAirlines", True),
        "max_stops": item.get("maxStops", 0),
        "departure_hour_range": item.get("departureHourRange"),
        "arrival_hour_range": item.get("arrivalHourRange"),
