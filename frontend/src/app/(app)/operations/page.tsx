...
function TranslatorPanel({
  ...
  const handleTranslate = async () => {
    ...
    setLoading(true);
    try {
      const res = await translateClientRequest({ vagueRequest: input.trim() });
      setResult(res);
    } catch {
      /* swallow */
    } finally {
  ...
  const load = async () => {
    ...
    try {
      const [dash, rems] = await Promise.all([
        getOperationsDashboard(),
        getReminders({ status: 'pending', dueBefore: new Date().toISOString() }),
      ]);
      setDashboard(dash);
      setReminders(rems);
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-32">
        ...
        <span className="ml-3 text-slate-500">Loading operations dashboard...</span>
      </div>
    );
  }
    ...
    <div className="max-w-7xl">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Track vendor requests, approvals, and trip readiness
          </p>
        </div>
        <div className="flex gap-2">
          <Link
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 text-xs text-slate-500">
                        {trip.clientName || 'No client'} · Departs{' '}
          ...
          {/* Translator */}
          <TranslatorPanel onCreateRequest={handleCreateFromTranslation} />
