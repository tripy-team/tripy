 ...
 * Dashboard UI.
 *
 * `initialData` is the server-rendered payload from the page Server Component.
 * When present, the data is already in the HTML on first paint — no client
 * fetch, no spinner, no cold-start wait. When it's null (cookie missing/expired,
 * or a server error), we fall back to fetching on mount exactly as before, so
 * this is never worse than the original behavior.
 */
export default function DashboardClient({ initialData = null }: { initialData?: DashboardData | null }) {
  ...
  // Only fetch on mount when the server didn't already provide the data.
  useEffect(() => {
