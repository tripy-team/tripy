...
export default function DatePickerCustom({ label }: DatePickerCustomProps) {
	const [selectedDate, setSelectedDate] = useState<DateValue | null>(
		parseDate(new Date().toISOString().split("T")[0]),
	);

	return (
		<div className="flex w-full justify-center rounded-md border border-gray-300">
			<DatePicker
				value={selectedDate}
			...
			>
				...
				<Group className="flex w-full items-center justify-between px-3 pb-3 font-semibold">
					<DateInput className="flex flex-1 flex-wrap rounded-md bg-white">
						{(segment) => (
							<DateSegment
								segment={segment}
					...
					<Button className="flex items-center rounded-md p-1 text-slate-950 hover:bg-gray-100">
									...
									{(date) => (
										<CalendarCell
											...
											className={({ isSelected, isFocused, isOutsideMonth }) =>
												`flex h-9 w-9 items-center justify-center rounded-full ${isSelected ? "bg-slate-950 text-white" : ""} ${isFocused ? "ring-2 ring-slate-400" : ""} ${isOutsideMonth ? "text-slate-300" : "text-slate-900"} hover:bg-gray-100`
											}
										/>
									)}
								</CalendarGridBody>
