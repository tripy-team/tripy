...
interface TripChatbotBoxProps {
  onExtract: (info: ExtractedTripInfo) => void;
}

export default function TripChatbotBox({ onExtract }: TripChatbotBoxProps) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
      ...
      {/* Messages */}
      <div className="h-64 overflow-y-auto p-4 space-y-4 bg-slate-50 rounded-xl mb-4 border border-slate-100">
      ...
      <div className="space-y-2">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent text-sm"
          />
          <button
            onClick={handleSend}
            disabled={!input.trim() || isTyping}
            className="px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center shadow-sm"
          >
